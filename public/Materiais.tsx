...
export default function Materiais() {
  ...
  const [historico, setHistorico] = useState<
    Array<{ alunoId: string; destinatario: string; data: string }>
  >([]);
  ...
  async function abrir(r: Registro) {
    ...
    try {
      const [ref, links] = await Promise.all([
        api<{ alunos: Aluno[] }>('/referencias'),
        api<typeof historico>(`/materiais/${r.id}/compartilhamentos`),
      ]);
      setAlunos(ref.alunos);
      setHistorico(links);
    } catch (e) {
  ...
  async function anexar(arquivo?: File) {
    ...
    try {
      ...
      await api(`/materiais/${selecionado.id}/anexo`, 'POST', { nome: arquivo.name, base64 });
      setMensagem('Arquivo salvo.');
      setTemAnexo(true);
  ...
  async function compartilhar() {
    ...
    setErro('');
    try {
      const resultado = await api<{ caminho: string }>(
        `/materiais/${selecionado.id}/compartilhar`,
        'POST',
        { alunoId, destinatario },
      );
  ...
  return (
    <>
      ...
      <Cadastro
        ...
        tipo="materiais"
        campos={[
          { nome: 'nome', rotulo: 'Título' },
          {
            nome: 'tipo',
            rotulo: 'Tipo',
            opcoes: ['Lista de exercícios', 'PDF', 'Imagem', 'Avaliação', 'Vídeo', 'Link'],
          },
          { nome: 'disciplinaId', rotulo: 'Disciplina', fonte: 'disciplinas' },
          {
            nome: 'conteudoId',
            rotulo: 'Conteúdo',
            fonte: 'conteudos',
            depende: 'disciplinaId',
            opcional: true,
          },
          { nome: 'aulaId', rotulo: 'Aula associada', fonte: 'aulas', opcional: true },
          { nome: 'link', rotulo: 'Link externo', opcional: true },
          { nome: 'descricao', rotulo: 'Descrição', tipo: 'area', opcional: true },
          { nome: 'status', rotulo: 'Situação', opcoes: ['Ativo', 'Inativo'] },
        ]}
        acoes={(r) => <button onClick={() => abrir(r)}>Arquivo e compartilhamento</button>}
      />
      {selecionado && (
        <section className="cadastro">
          <h3>{String(selecionado.dados.nome)}</h3>
          {erro && (
          ...
          <label>
            Arquivo (PDF, PNG ou JPEG; até 5 MB)
            <input
              type="file"
          ...
          {temAnexo ? (
            <p>
              <a href={`/api/materiais/${selecionado.id}/anexo`}>Baixar arquivo anexado</a>
            </p>
          ) : (
            <p>Nenhum arquivo anexado.</p>
          )}
          {!!selecionado.dados.link && (
            <p>
              <a href={String(selecionado.dados.link)} target="_blank" rel="noreferrer">
                Abrir link externo
              </a>
            </p>
          )}
          ...
          <button disabled={ocupado || !alunoId} onClick={compartilhar}>
            Gerar link
          </button>
          {link && (
            <label>
          ...
          <h4>Histórico de compartilhamentos</h4>
          {historico.map((h, i) => (
            <p key={i}>
              {new Date(h.data).toLocaleString('pt-BR')} —{' '}
              {alunos.find((a) => a.id === h.alunoId)?.nome ?? h.alunoId} — {h.destinatario}
            </p>
          ))}
