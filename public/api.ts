export async function api<T>(
  ...
  try {
    ...
    throw new Error('Sem resposta do servidor. Confira se o backend está rodando.');
  }
  const resultado = await resposta.json().catch(() => null);
  if (resposta.status === 401 && caminho !== '/auth/entrar' && !ignorarSessaoEncerrada)
    window.dispatchEvent(new Event('sessao-encerrada'));
  if (!resposta.ok || resultado === null)
