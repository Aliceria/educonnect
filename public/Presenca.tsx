      ...
      <Cadastro
        ...
        rodape={(lista, refs) => (
          <>
            ...
            <p>
              Cancelamentos, reagendamentos e faltas do professor ficam fora do cálculo. Faltas
              justificadas do aluno contam como falta.
            </p>
            {refs.alunos?.map((aluno) => {
              const registros = lista.filter((r) => r.dados.alunoId === aluno.id);
              const presentes = registros.filter((r) =>
                ['Presente', 'Realizada', 'Reposição realizada'].includes(String(r.dados.status)),
              ).length;
              const faltas = registros.filter((r) =>
                ['Falta', 'Falta justificada'].includes(String(r.dados.status)),
              ).length;
              return (
                <p key={aluno.id}>
                  {aluno.nome}:{' '}
                  {presentes + faltas
                    ? `${((presentes / (presentes + faltas)) * 100).toFixed(1)}% (${presentes} presenças, ${faltas} faltas)`
                    : 'sem registros'}
                </p>
              );
            })}
