...
const opcoes = {
  ...
  responsavel: 'Para o responsável',
};
export default function Relatorios() {
  ...
  async function gerar() {
    ...
    setResultado(null);
    try {
      const p = new URLSearchParams({ tipo, inicio, fim, alunoId }).toString();
      setResultado(await api(`/relatorios?${p}`));
      setConsulta(p);
    } catch (e) {
    ...
    <div className="cadastro">
      ...
      <div className="campos">
        ...
        <label>
          ...
          <select value={alunoId} onChange={(e) => setAlunoId(e.target.value)}>
            <option value="">{tipo === 'responsavel' ? 'Selecione um aluno' : 'Todos'}</option>
            {alunos.map((a) => (
      ...
      {resultado && (
        <>
          ...
          {resultado.avisos.map((r, i) => (
            <p className="observacao" key={i}>
              {r}
            </p>
          ))}
          <p>
            <a href={`/api/relatorios/pdf?${consulta}`} download="relatorio.pdf">
