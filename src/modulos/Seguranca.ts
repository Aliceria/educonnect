...
export function seguranca(banco: Banco) {
  ...
  function instalado() {
    return Number(banco.db.prepare('SELECT count(*) AS n FROM usuarios').get()!.n) > 0;
  }
  function listar(): Usuario[] {
    return banco.db
      .prepare('SELECT dados FROM usuarios')
      .all()
      .map((linha) => JSON.parse(String(linha.dados)));
  }
  ...
  function validarUsuario(d: Dados, anterior?: Usuario): Usuario {
    ...
    if (
      ...
      d.permissoes.some((p) => typeof p !== 'string' || !modulos.includes(p))
    )
    ...
    if (portal) {
      const aluno = banco.db
        .prepare("SELECT dados FROM registros WHERE tipo='alunos' AND id=?")
        .get(alunoId);
      if (
        !aluno ||
        JSON.parse(String(aluno.dados)).status !== 'Ativo' ||
        (d.perfil === 'Responsável' && !JSON.parse(String(aluno.dados)).responsavel)
      )
        throw new ErroCadastro('Selecione um aluno ativo com responsável, quando necessário.');
    }
  ...
  async function criar(entrada: unknown, autor?: Usuario) {
    ...
    if (banco.db.prepare('SELECT id FROM usuarios WHERE email = ?').get(usuario.email))
      throw new ErroCadastro('E-mail já cadastrado.', 409);
    const codigo = randomBytes(24).toString('hex');
    banco.db
      .prepare('INSERT INTO usuarios (id,email,dados,senha,recuperacao) VALUES (?,?,?,?,?)')
      .run(usuario.id, usuario.email, JSON.stringify(usuario), senha, resumo(codigo));
    banco.evento(autor ?? usuario, 'seguranca', 'Criação de usuário', usuario.id, {
      nome: usuario.nome,
  ...
  async function configurar(entrada: unknown) {
    if (instalado() || criandoInicial)
      throw new ErroCadastro('O administrador inicial já foi configurado.', 409);
    criandoInicial = true;
    try {
      return await criar({
        ...objeto(entrada),
  ...
  function autorizado(usuario: Usuario) {
    ...
    if (usuario.perfil === 'Professor') {
      const linha = banco.db
        .prepare('SELECT cadastro FROM professores WHERE id = ?')
    ...
    if (['Aluno', 'Responsável'].includes(usuario.perfil)) {
      ...
      if (
        !aluno ||
        JSON.parse(String(aluno.dados)).status !== 'Ativo' ||
        (usuario.perfil === 'Responsável' && !JSON.parse(String(aluno.dados)).responsavel)
      )
        return false;
    }
    return true;
  ...
  async function entrar(entrada: unknown, ip: string) {
    ...
    if (!atual || atual.senha !== linha.senha || atual.dados !== linha.dados)
      throw new ErroCadastro('O cadastro mudou durante a entrada. Tente novamente.', 401);
    ...
    banco.db
      .prepare('INSERT INTO sessoes VALUES (?,?,?,?)')
      .run(resumo(token), usuario.id, Date.now(), Date.now());
  ...
  function sessao(token: string, minutos: number): Usuario {
    ...
    if (
      !linha ||
      Date.now() - Number(linha.ultimaAtividade) > minutos * 60000 ||
      Date.now() - Number(linha.criada) > 12 * 3600000
    ) {
    ...
    banco.db
      .prepare('UPDATE sessoes SET ultimaAtividade = ? WHERE token = ?')
      .run(Date.now(), resumo(token));
    return usuario;
  }
  ...
  function editar(id: string, entrada: unknown, autor: Usuario) {
    ...
    if (
      ...
      anterior.ativo &&
      (usuario.perfil !== 'Administrador' || !usuario.ativo) &&
      listar().filter((u) => u.perfil === 'Administrador' && u.ativo).length <= 1
    )
      throw new ErroCadastro('Mantenha pelo menos um administrador ativo.');
    if (listar().some((u) => u.email === usuario.email && u.id !== id))
      throw new ErroCadastro('E-mail já cadastrado.', 409);
    banco.db
      .prepare('UPDATE usuarios SET email=?, dados=? WHERE id=?')
      .run(usuario.email, JSON.stringify(usuario), id);
    banco.db.prepare('DELETE FROM sessoes WHERE usuarioId=?').run(id);
    banco.evento(autor, 'seguranca', 'Permissões alteradas', id, {
      antes: anterior,
      depois: usuario,
    });
    return usuario;
  ...
  async function recuperar(entrada: unknown, ip: string) {
    ...
    const linha = banco.db.prepare('SELECT * FROM usuarios WHERE email=?').get(email);
    if (!linha || resumo(codigo) !== linha.recuperacao)
      throw new ErroCadastro('E-mail ou código de recuperação inválido.', 400);
    const senha = await hashSenha(validarSenha(d));
    const novoCodigo = randomBytes(24).toString('hex');
    const resultado = banco.db
      .prepare('UPDATE usuarios SET senha=?, recuperacao=? WHERE id=? AND recuperacao=?')
  ...
  function perfil(entrada: unknown, usuario: Usuario) {
    const d = objeto(entrada);
    const novo = validarUsuario(
      { ...usuario, nome: d.nome, email: d.email, telefone: d.telefone },
      usuario,
    );
    if (listar().some((u) => u.id !== usuario.id && u.email === novo.email))
      throw new ErroCadastro('E-mail já cadastrado.', 409);
    banco.db
      .prepare('UPDATE usuarios SET email=?,dados=? WHERE id=?')
      .run(novo.email, JSON.stringify(novo), usuario.id);
