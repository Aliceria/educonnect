...
export function validarPresenca(
  ...
  const existente = banco.db
    .prepare(
      "SELECT id FROM registros WHERE tipo='presencas' AND json_extract(dados,'$.aulaId')=? AND id<>?",
    )
    .get(aulaId, id ?? '');
  if (existente)
    throw new ErroCadastro(
      'A frequência dessa aula já foi registrada. Solicite a edição do registro existente.',
      409,
    );
  ...
  if (
    status === 'Reposição realizada' &&
    (!aula.originalId ||
      aula.originalId === aula.id ||
      !aulas.some((a) => a.id === aula.originalId && a.alunoId === aula.alunoId))
  )
    throw new ErroCadastro(
      'A reposição precisa estar vinculada a outra aula do mesmo aluno no agendamento.',
    );
  return {
    aulaId,
    ...
    status,
    justificativa,
    originalId: aula.originalId ?? '',
  };
}
export function calcularFrequencia(registros: Dados[]) {
  const presentes = registros.filter((r) =>
    ['Presente', 'Realizada', 'Reposição realizada'].includes(String(r.status)),
  ).length;
  const faltas = registros.filter((r) =>
    ['Falta', 'Falta justificada'].includes(String(r.status)),
  ).length;
  return {
    presentes,
    faltas,
    percentual:
      presentes + faltas ? Math.round((presentes / (presentes + faltas)) * 10000) / 100 : null,
  };
}
