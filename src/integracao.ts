import { situacaoPagamento } from './modulos/Pagamentos.ts';
...
export function fontesDoBanco(banco: Banco): Fontes {
  function registros(tipo: string) {
    return banco.db
      .prepare('SELECT id,dados FROM registros WHERE tipo=?')
      .all(tipo)
      .map((r) => ({ id: String(r.id), ...JSON.parse(String(r.dados)) }));
  }
  return {
    alunos: () =>
    ...
    aulas: () => {
      const planos = new Map(registros('planejamentos').map((p) => [p.aulaId, p]));
      return registros('aulas').map((a) => {
        const plano = planos.get(a.id);
        return {
          id: a.id,
          alunoId: a.alunoId,
          professorId: a.professorId,
          data: `${a.data}T${a.hora}`,
          duracaoMinutos: a.duracaoMinutos,
          status: a.status,
          conteudos: plano?.trabalhado || a.conteudos || '',
          originalId: a.originalId,
        };
      });
    },
    pagamentos: () =>
      registros('pagamentos').map((p) => ({
        id: p.id,
        alunoId: p.alunoId,
        professorId: p.professorId,
        vencimento: p.vencimento,
        recebidoEm: p.recebidoEm || undefined,
        valor: p.valor,
        status: situacaoPagamento(p),
      })),
  };
}
