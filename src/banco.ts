...
export function abrirBanco(caminho: string) {
  ...
  function buscar(tipo: string, id: string, usuario: Usuario) {
    ...
    return {
      ...
      autorId: String(linha.autorId),
      versao: Number(linha.versao),
      dados: JSON.parse(String(linha.dados)) as Dados,
    };
  }
  function salvar(tipo: string, dados: Dados, usuario: Usuario, id?: string, versao?: number) {
    ...
    if (anterior && anterior.versao !== versao)
      throw new ErroCadastro('Registro alterado em outra janela. Atualize a lista.', 409);
    const registro = {
      id: id ?? randomUUID(),
      tipo,
      autorId: anterior?.autorId ?? usuario.id,
      versao: (anterior?.versao ?? 0) + 1,
      dados,
    };
    db.prepare(
      'INSERT INTO registros (id,tipo,autorId,versao,dados) VALUES (?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET versao=excluded.versao,dados=excluded.dados',
    ).run(registro.id, tipo, registro.autorId, registro.versao, JSON.stringify(dados));
    evento(usuario, tipo, anterior ? 'Alteração' : 'Cadastro', registro.id, {
      antes: anterior?.dados ?? null,
      depois: dados,
    });
    return registro;
  }
...
export function objeto(valor: unknown): Dados {
  if (!valor || typeof valor !== 'object' || Array.isArray(valor))
    throw new ErroCadastro('Dados inválidos.');
  return valor as Dados;
}
export function texto(d: Dados, campo: string, obrigatorio = true, limite = 2000): string {
  if (typeof d[campo] !== 'string' || (obrigatorio && !d[campo].trim()) || d[campo].length > limite)
    throw new ErroCadastro(`Confira o campo ${campo}.`);
  return d[campo].trim();
}
...
export function data(d: Dados, campo: string): string {
  ...
  if (
    ...
    throw new ErroCadastro(`Data inválida em ${campo}.`);
  return valor;
}
