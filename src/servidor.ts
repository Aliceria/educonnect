    ...
    try {
      ...
      if (publico && metodo === 'GET') {
        ...
        if (['Aluno', 'Responsável'].includes(leitor.perfil)) {
          if (leitor.alunoId !== link.alunoId || leitor.perfil !== link.destinatario)
            throw new ErroCadastro('Material não compartilhado com esta conta.', 403);
        } else {
          permitir(leitor, 'materiais');
          banco.buscar('materiais', String(link.materialId), leitor);
        }
        banco.evento(
          leitor,
          'materiais',
          'Material compartilhado acessado',
          String(link.materialId),
        );
      ...
      if (solicitacao && metodo === 'PUT') {
        responder(200, responderPrivacidade(banco, usuario, solicitacao[1], corpo));
        return;
      }
      ...
      if (caminho === '/api/meus-dados' && metodo === 'GET') {
        responder(200, meusDados(banco, usuario));
        return;
      }
      if (caminho === '/api/privacidade' && metodo === 'POST') {
        responder(201, solicitarPrivacidade(banco, usuario, corpo));
        return;
      }
      if (['Aluno', 'Responsável'].includes(usuario.perfil))
        throw new ErroCadastro(
          'Esta conta acessa apenas sua área e materiais compartilhados.',
          403,
        );
      ...
      if (caminho === '/api/dashboard' && metodo === 'GET') {
        permitir(usuario, 'dashboard');
        responder(200, resumoDashboard(banco, usuario, fontes));
        return;
      }

      const ficha = /^\/api\/alunos\/([a-f0-9-]+)\/historico$/.exec(caminho);
      if (ficha && metodo === 'GET') {
        permitir(usuario, 'alunos');
        responder(200, historicoAluno(banco, usuario, ficha[1]));
        return;
      }
      const repeticao = /^\/api\/aulas\/([a-f0-9-]+)\/repetir$/.exec(caminho);
      if (repeticao && metodo === 'POST') {
        permitir(usuario, 'agendamento');
        responder(
          201,
          transacao(() => {
            const novas = repetirAula(banco, usuario, repeticao[1], corpo);
            const pacoteId = novas[0]?.dados.pacoteId;
            if (pacoteId) {
              const saldo = saldoPacote(banco, usuario, String(pacoteId));
              if (saldo.restantes < saldo.agendadas)
                throw new ErroCadastro('O pacote não tem saldo para todas as repetições.', 409);
            }
            return novas;
          }),
        );
      ...
      if (reagendamento && metodo === 'POST') {
        permitir(usuario, 'agendamento');
        responder(
          201,
          transacao(() => reagendar(banco, usuario, reagendamento[1], corpo)),
        );
        return;
      }
      const envio = /^\/api\/comunicacoes\/([a-f0-9-]+)\/abrir$/.exec(caminho);
      if (envio && metodo === 'GET') {
        permitir(usuario, 'comunicacao');
        responder(200, prepararEnvio(banco, usuario, envio[1]));
        return;
      }
      if (caminho === '/api/saldos' && metodo === 'GET') {
        permitir(usuario, 'pagamentos');
        permitir(usuario, 'financeiro');
        responder(
          200,
          banco.listar('pacotes', usuario).map((p) => saldoPacote(banco, usuario, p.id)),
        );
        return;
      }
      ...
      const rotas: Record<string, { modulo: string; validar: (d: Dados, id?: string) => Dados }> = {
        ...
        planejamentos: {
          modulo: 'planejamento',
          validar: (d, id) => validarPlanejamento(d, banco, usuario, id),
        },
        modelos: { modulo: 'planejamento', validar: validarModelo },
        acompanhamentos: {
          modulo: 'acompanhamento',
          validar: (d) => validarAcompanhamento(d, banco, usuario),
        },
        necessidades: {
          modulo: 'acompanhamento',
          validar: (d) => validarNecessidade(d, banco, usuario),
        },
        pacotes: { modulo: 'pagamentos', validar: (d, id) => validarPacote(d, banco, usuario, id) },
        pagamentos: { modulo: 'pagamentos', validar: (d) => validarPagamento(d, banco, usuario) },
        comunicacoes: {
          modulo: 'comunicacao',
          validar: (d) => validarComunicacao(d, banco, usuario),
        },
      ...
      if (registro && Object.hasOwn(rotas, registro[1])) {
        ...
        if (metodo === 'GET') {
          ...
          responder(
            200,
            id ? resultados.find((r) => r.id === banco.buscar(tipo, id, usuario).id) : resultados,
          );
          return;
        }
          ...
          responder(
            ...
            transacao(() => {
              ...
              if (
                tipo === 'alunos' &&
                id &&
                banco.buscar(tipo, id, usuario).dados.professorId !== dados.professorId &&
                banco.db
                  .prepare(
                    "SELECT id FROM registros WHERE json_extract(dados,'$.alunoId')=? LIMIT 1",
                  )
                  .get(id)
              )
                throw new ErroCadastro(
                  'O aluno já tem registros vinculados. Mantenha o professor para preservar o acesso ao histórico.',
                );
              if (dados.alunoId) {
                const aluno = referencias(fontes, usuario).alunos.find(
                  (a) => a.id === dados.alunoId,
                );
              ...
              if (tipo === 'aulas' || tipo === 'presencas') {
                ...
                const linha = banco.db
                  .prepare("SELECT id FROM registros WHERE tipo='aulas' AND id=?")
                  .get(aulaId);
                if (linha) {
                  ...
                  if (tipo === 'presencas')
                    banco.salvar(
                      ...
                      validarAula(
                        {
                          ...aula.dados,
                          status: dados.status === 'Presente' ? 'Realizada' : dados.status,
                    ...
                    const presenca = banco.db
                      .prepare(
                        "SELECT id FROM registros WHERE tipo='presencas' AND json_extract(dados,'$.aulaId')=?",
                      )
                      .get(aulaId);
                    if (presenca) {
                      banco.db.prepare('DELETE FROM registros WHERE id=?').run(String(presenca.id));
                      banco.evento(
                        usuario,
                        'presencas',
                        'Frequência removida ao reabrir aula',
                        String(presenca.id),
                      );
                    }
                  } else {
                    const p = banco.db
                      .prepare(
                        "SELECT id,versao FROM registros WHERE tipo='presencas' AND json_extract(dados,'$.aulaId')=?",
                      )
                      .get(aulaId);
                    banco.salvar(
                      'presencas',
                      {
                        ...
                        status: dados.status,
                        justificativa: dados.justificativa ?? '',
                        originalId: dados.originalId,
                      },
                      usuario,
                      p ? String(p.id) : undefined,
                      p ? Number(p.versao) : undefined,
                    );
                  }
                  if (aula.dados.pacoteId) {
                    const saldo = saldoPacote(banco, usuario, String(aula.dados.pacoteId));
                    if (saldo.restantes < saldo.agendadas)
                      throw new ErroCadastro('O pacote não tem aulas suficientes.', 409);
                  }
                }
              }
              if (tipo === 'pacotes') {
                const saldo = saldoPacote(banco, usuario, r.id);
                if (saldo.restantes < saldo.agendadas)
                  throw new ErroCadastro(
                    'Quantidade menor que as aulas já utilizadas ou agendadas.',
                    409,
                  );
              }
              return r;
